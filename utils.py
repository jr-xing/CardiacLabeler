...
def loadStrainMat(filename):
    ...
    if 'xs' in datamat.keys():
        tos = datamat['xs'][::-1]
        tos18_new = datamat.get('xs18_new', None)
        tos126_new = datamat.get('xsfullRes_new', None)
    elif 'TOSAnalysis' in datamat.keys():
        if 'xs' not in datamat['TOSAnalysis'].keys():
            # Old format (before version 1.4)
            try:
            ...
            try:
                ...
                tos = None
            try:
                tos18_new = datamat['TOSAnalysis'].TOS18_new[::-1]
                tos126_new = datamat['TOSAnalysis'].TOSfullRes_new[::-1]
            except:
                tos18_new = None
                tos126_new = None 
    else:
    ...
    return {'strainMat': EccDatum, 'TOS': tos, 'TOS18_new': tos18_new, 'TOS126_new': tos126_new,
